...
uint8_t arduinoLoRaWAN::sleep(unsigned long time_ms){
	uint8_t status;
	char ans1[15];
	char ans2[15];
	...
	// create "sys sleep" command
	sprintf_P(_command,(char*)F("sys sleep %lu\r\n"), time_ms);

	// the ok answer is issued after the sleep time has elapsed
	// create "ok" answer
	sprintf_P(ans1,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[0])));
	// create "invalid_param" answer
	sprintf_P(ans2,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[1])));
	...
	//send command and wait for ans
	status = sendCommand(_command,ans1, ans2,time_ms + 3000l);
	if (status == 1)
	{
		return LORAWAN_ANSWER_OK;
	}
